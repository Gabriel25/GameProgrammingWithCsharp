...
namespace ProgrammingAssignment4
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        //used to fix the bug that makes the teddy fly off screen
        //when there are no more pick ups and it's left clicked
        public static bool morePickUpsExist=false;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            else if (mouse.RightButton == ButtonState.Released)
            {
                ...
                if (rightClickStarted)
                {
                    ...
                    //stop collecting when there are no more pickups
                    if (pickups.Count == 0)
                        teddy.Collecting = false;

                    // STUDENTS: add a new pickup to the end of the list of pickups
                    if(mouse.X < WINDOW_WIDTH && mouse.X>=0 && mouse.Y<WINDOW_HEIGHT && mouse.Y >= 0)
